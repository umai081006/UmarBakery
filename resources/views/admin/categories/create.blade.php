...
<div class="space-y-8 max-w-2xl">
    <div>
        <div class="flex items-center gap-2">
            ...
            <h1 class="text-3xl font-extrabold text-stone-900 tracking-tight font-sans">Tambah Kategori</h1>
        </div>
        <p class="text-sm text-stone-500 mt-1">Tambahkan kategori roti baru.</p>
    </div>
    ...
    <div class="bg-white rounded-3xl border border-stone-200 p-6 sm:p-8 shadow-sm">
        <form method="POST" action="{{ route('admin.categories.store') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="Contoh: Roti Manis, Cake, Pastry" class="w-full px-4 py-2.5 rounded-xl border border-stone-200 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 text-sm">
                @error('name')
            ...
            <div>
                <label for="image" class="block text-xs font-semibold text-stone-500 uppercase tracking-wider mb-2">Gambar Kategori (Opsional)</label>
                ...
                <p class="text-[10px] text-stone-400 mt-1">Rekomendasi ukuran persegi (1:1), Maksimal 2MB</p>
                @error('image')
                    <p class="text-rose-600 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
